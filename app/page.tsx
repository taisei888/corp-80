...
const NODE_COLORS = ["#818cf8", "#a78bfa", "#c084fc", "#67e8f9"];
const CONN_DIST   = 170;
const NODE_COUNT  = 85;
  ...
  useEffect(() => {
    ...
    const render = (ts: number) => {
      ...
      for (const n of nodes) {
        ...
        ctx.fillStyle   = n.glow > 0.4 ? "#fff" : n.color;
        ctx.beginPath();
      ...
      pulses = pulses.filter(p => {
        ...
        for (let k = 0; k < 5; k++) {
          ...
          const tx  = ni.x + (nj.x - ni.x) * tt;
          const ty  = ni.y + (nj.y - ni.y) * tt;
          const ta  = (1 - k / 5) * 0.9;
          ctx.globalAlpha = ta;
          ctx.fillStyle   = k === 0 ? "#fff" : p.color;
          ctx.shadowBlur  = k === 0 ? 14 : 0;
          ctx.shadowColor = p.color;
          ctx.beginPath();
          ctx.arc(tx, ty, k === 0 ? 2.8 : 1.5 - k * 0.2, 0, Math.PI * 2);
          ctx.fill();
          ctx.shadowBlur  = 0;
          ctx.globalAlpha = 1;
        }
  ...
  return (
    <canvas
      ...
      style={{ position: "fixed", inset: 0, width: "100vw", height: "100vh",
        zIndex: 0, display: "block", background: "#04060f" }}
    />
  );
}
    ...
    <>
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <nav style={{
          ...
          padding: "0 48px",
          background: navScrolled ? "rgba(4,6,15,0.85)" : "transparent",
          ...
          <button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
            style={{ background: "none", border: "none", cursor: "pointer",
              fontSize: 22, fontWeight: 900, color: "#fff", letterSpacing: "-0.03em" }}>
            80
          </button>
        ...
        <section style={{ minHeight: "100vh", display: "flex", alignItems: "center",
          ...
          <div style={{ textAlign: "center", maxWidth: 820 }}>
            <p style={{ fontSize: 12, letterSpacing: "0.22em", color: "#818cf8",
            ...
            <h1 style={{ fontSize: "clamp(52px, 8.5vw, 100px)", fontWeight: 800,
              color: "#fff", lineHeight: 1.12, letterSpacing: "-0.04em", marginBottom: 32,
              animation: "fade-up 0.9s cubic-bezier(0.16,1,0.3,1) 0.25s both" }}>
              人の知覚を、<br />
              <span style={{ color: "#818cf8" }}>ソフトウェアで</span><br />
              拡張する。
            </h1>
            <p style={{ fontSize: 17, color: "rgba(255,255,255,0.45)", lineHeight: 1.85, marginBottom: 52,
            ...
            <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap",
              ...
              <button className="btn-primary-dark" onClick={() => scrollTo("business")}>
                事業内容を見る
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                  <path d="M5 12h14M12 5l7 7-7 7" />
                </svg>
              </button>
              <button className="btn-ghost-dark" onClick={() => scrollTo("contact")}>
                お問い合わせ
              </button>
            </div>
            <div style={{ marginTop: 80, display: "flex", flexDirection: "column",
              alignItems: "center", gap: 10, color: "rgba(255,255,255,0.2)",
          ...
          <div style={{ maxWidth: 1160, margin: "0 auto" }}>
            <div className="sr" style={{ marginBottom: 64 }}>
              <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.2em",
                color: "#818cf8", marginBottom: 16, textTransform: "uppercase" }}>Business</div>
              <h2 style={{ fontSize: "clamp(28px, 4vw, 48px)", fontWeight: 800,
                letterSpacing: "-0.03em", lineHeight: 1.2, color: "#fff" }}>3つの事業領域</h2>
              <p style={{ fontSize: 16, color: "rgba(255,255,255,0.45)", lineHeight: 1.9, marginTop: 14, maxWidth: 420 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }}>
              {[
                {
                  n: "01", color: "#818cf8", label: "SaaS Package",
                  icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#818cf8" strokeWidth="1.8"><rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8M12 17v4"/></svg>,
                  title: "パッケージ販売", sub: "LENDS AI など",
                  desc: "自社開発のSaaSプロダクトを提供。LENDS AIは組織コンディションをスマホアンケート×AIで可視化し、人事課題の早期発見を支援します。",
                  delay: "0s",
                },
                {
                  n: "02", color: "#a78bfa", label: "AI Development",
                  icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#a78bfa" strokeWidth="1.8"><path d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17H3a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2h-2"/></svg>,
                  title: "AI受託開発", sub: "",
                  desc: "ChatGPT・LLM・RAGを活用した業務効率化ツール、データ分析システム、AIチャットボットなどをオーダーメイドで開発します。",
                  delay: "0.1s",
                },
                {
                  n: "03", color: "#67e8f9", label: "Web Design",
                  icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#67e8f9" strokeWidth="1.8"><rect x="3" y="4" width="18" height="14" rx="2"/><path d="M7 8h2M7 12h2M13 8h4M13 12h4"/></svg>,
                  title: "HP制作デザイン", sub: "",
                  desc: "ブランドの価値を最大化するWebサイト制作。戦略的なUI/UXデザインと高品質な実装で、ビジネスの成果につながるサイトを届けます。",
                  delay: "0.2s",
                },
              ].map((item) => (
                <div key={item.n} className="sr" style={{ ...card, transitionDelay: item.delay, transition: "transform 0.28s ease, background 0.28s ease" }}
                  ...
                  <div style={{ fontSize: 11, fontWeight: 800, color: "rgba(255,255,255,0.2)",
                  ...
                  <h3 style={{ fontSize: 20, fontWeight: 700, marginBottom: 12, lineHeight: 1.35,
                    color: "#fff" }}>
                    {item.title}
                    {item.sub && <><br /><span style={{ fontSize: 13, fontWeight: 400, color: "rgba(255,255,255,0.3)" }}>{item.sub}</span></>}
                  </h3>
                  <p style={{ fontSize: 14, lineHeight: 1.9, color: "rgba(255,255,255,0.45)" }}>{item.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section id="product" style={{ padding: "120px 48px" }}>
          <div style={{ maxWidth: 1160, margin: "0 auto" }}>
            <div className="sr" style={{ marginBottom: 64 }}>
              <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.2em",
                color: "#818cf8", marginBottom: 16, textTransform: "uppercase" }}>Featured Product</div>
              <h2 style={{ fontSize: "clamp(28px, 4vw, 48px)", fontWeight: 800,
                letterSpacing: "-0.03em", lineHeight: 1.2, color: "#fff" }}>LENDS AI</h2>
              <p style={{ fontSize: 16, color: "rgba(255,255,255,0.45)", marginTop: 10 }}>組織コンディション可視化プラットフォーム</p>
            </div>

            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 72, alignItems: "center" }}>
              <div className="sr">
                <h3 style={{ fontSize: "clamp(20px, 2.8vw, 30px)", fontWeight: 700,
                  lineHeight: 1.6, marginBottom: 20, color: "#fff" }}>
                  スマホアンケート × AI分析で、<br />従業員の「今」を見える化する。
                </h3>
                <p style={{ fontSize: 15, color: "rgba(255,255,255,0.45)", lineHeight: 1.9, marginBottom: 32 }}>
                ...
                <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: 10, marginBottom: 36 }}>
                  {["月次スマホアンケート × AI組織診断","8タイプ組織コンディション可視化","1on1面談記録・アシスト機能","採用AI・採用KPI管理","採用アセスメント（心理・論理・コミュ・敬語）","要面談候補者アラートメール通知"].map(f => (
                    <li key={f} style={{ display: "flex", gap: 10, alignItems: "center", fontSize: 14, color: "rgba(255,255,255,0.55)" }}>
                ...
                <a href="https://www.lens-ai.jp" target="_blank" rel="noopener noreferrer"
                  className="btn-primary-dark" style={{ display: "inline-flex", textDecoration: "none" }}>
                  LENDS AI サイトへ
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
              ...
              <div className="sr" style={{ transitionDelay: "0.15s" }}>
                <div style={{ ...card, padding: 0, overflow: "hidden" }}>
                  <div style={{ background: "rgba(255,255,255,0.04)", padding: "14px 20px",
                    ...
                    <div style={{ marginLeft: 10, fontSize: 11, color: "rgba(255,255,255,0.3)" }}>LENDS AI — 組織ダッシュボード</div>
                  </div>
                  <div style={{ padding: 28 }}>
                    <div style={{ display: "flex", gap: 12, marginBottom: 24 }}>
                      {[{val:"87",unit:"pt",label:"組織スコア",color:"#818cf8"},{val:"+12",unit:"",label:"先月比",color:"#34d399"},{val:"64",unit:"名",label:"回答者数",color:"#a78bfa"}].map(s => (
                    ...
                    <div style={{ fontSize: 10, color: "rgba(255,255,255,0.25)", marginBottom: 10, letterSpacing: "0.08em", textTransform: "uppercase" }}>月別コンディション推移</div>
                    <div style={{ display: "flex", alignItems: "flex-end", gap: 5, height: 80 }}>
                      {[52,68,60,78,72,85,87].map((h, i) => (
                        <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 5, height: "100%" }}>
                          <div style={{ width: "100%", height: `${h}%`,
                            background: i === 6 ? "linear-gradient(to top, #6366f1, #a78bfa)" : "rgba(129,140,248,0.25)",
                            borderRadius: "4px 4px 0 0", marginTop: "auto" }} />
                          <div style={{ fontSize: 9, color: "rgba(255,255,255,0.2)" }}>{["7月","8月","9月","10月","11月","12月","1月"][i]}</div>
                        </div>
                      ))}
                    </div>
        ...
        <section id="about" style={{ padding: "120px 48px" }}>
          <div style={{ maxWidth: 1160, margin: "0 auto" }}>
            <div className="sr" style={{ marginBottom: 64 }}>
              <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.2em",
                color: "#818cf8", marginBottom: 16, textTransform: "uppercase" }}>Philosophy</div>
              <h2 style={{ fontSize: "clamp(28px, 4vw, 48px)", fontWeight: 800,
              ...
              {[
                ...
                <div key={v.label} className="sr" style={{ ...card, transitionDelay: v.delay }}>
                  <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: "0.22em",
                    color: "#818cf8", textTransform: "uppercase", marginBottom: 18 }}>{v.label}</div>
                  <p style={{ fontSize: "clamp(17px, 2vw, 22px)", fontWeight: 700,
                    lineHeight: 1.55, color: "#fff", marginBottom: 16 }}>{v.title}</p>
                  <p style={{ fontSize: 14, color: "rgba(255,255,255,0.45)", lineHeight: 1.9 }}>{v.desc}</p>
                </div>
              ))}
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 4 }}>
              {[
                ...
                <div key={v.en} className="sr" style={{ ...card, transitionDelay: `${i*0.08}s`,
                  borderTop: "2px solid #6366f1", borderRadius: "0 0 16px 16px" }}>
                  <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: "0.2em",
                    color: "#818cf8", textTransform: "uppercase", marginBottom: 10 }}>{v.en}</div>
                  <div style={{ fontSize: 18, fontWeight: 700, color: "#fff", marginBottom: 10 }}>{v.jp}</div>
                  <div style={{ fontSize: 13, color: "rgba(255,255,255,0.4)", lineHeight: 1.85 }}>{v.desc}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <section id="contact" style={{ padding: "120px 48px 140px" }}>
          <div style={{ maxWidth: 600, margin: "0 auto" }}>
            <div className="sr" style={{ textAlign: "center", marginBottom: 52 }}>
              <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.2em",
                color: "#818cf8", marginBottom: 18, textTransform: "uppercase" }}>Contact</div>
              <h2 style={{ fontSize: "clamp(32px, 5vw, 56px)", fontWeight: 800,
                color: "#fff", lineHeight: 1.2, letterSpacing: "-0.03em", marginBottom: 18 }}>
                一緒に、未来を<br />作りませんか。
              </h2>
              <p style={{ fontSize: 15, color: "rgba(255,255,255,0.4)", lineHeight: 1.9 }}>
        ...
        <footer style={{ background: "rgba(0,0,0,0.5)", borderTop: "1px solid rgba(255,255,255,0.06)",
          ...
          <div>
            <div style={{ fontSize: 20, fontWeight: 900, color: "#fff" }}>80</div>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.25)", marginTop: 4 }}>合同会社80</div>
          </div>
            ...
            {([["事業内容","business"],["プロダクト","product"],["会社について","about"],["お問い合わせ","contact"]] as const).map(([l, id]) => (
              <button key={id}
                ...
                style={{ fontSize: 12, color: "rgba(255,255,255,0.3)", background: "none",
                  border: "none", cursor: "pointer", padding: 0, transition: "color 0.2s", fontFamily: "inherit" }}
                onMouseEnter={e => (e.currentTarget.style.color = "#fff")}
...
function ContactForm() {
  ...
  if (status === "done") return (
    <div style={{ textAlign: "center", padding: "32px 0" }}>
      <div style={{ width: 52, height: 52, borderRadius: "50%", background: "rgba(129,140,248,0.15)",
      ...
      <div style={{ fontSize: 17, fontWeight: 700, color: "#fff", marginBottom: 8 }}>送信完了しました</div>
    ...
    <form onSubmit={submit} style={{ display: "grid", gap: 14 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        {[{k:"company",label:"会社名",ph:"株式会社〇〇",req:true,type:"text"},{k:"name",label:"お名前",ph:"山田 太郎",req:true,type:"text"}].map(({ k,label,ph,req,type }) => (
          <div key={k}>
            <label style={lbl}>{label}{req && <span style={{ color:"#f87171",marginLeft:4 }}>*</span>}</label>
            <input required={req} type={type} placeholder={ph}
              value={form[k as keyof typeof form]} onChange={e => setForm({ ...form, [k]: e.target.value })}
              style={inp} onFocus={e => (e.target.style.borderColor="#818cf8")}
      ...
      <button type="submit" disabled={status==="sending"} className="btn-primary-dark"
        style={{ width:"100%", justifyContent:"center", marginTop:4, opacity:status==="sending"?0.7:1 }}>
        {status==="sending" ? "送信中..." : "送信する"}
      </button>
      {status==="error" && (
        <div style={{ fontSize:13, color:"#f87171", textAlign:"center" }}>送信に失敗しました。時間をおいて再度お試しください。</div>
      )}
    </form>
  );
}
